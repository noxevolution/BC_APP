...
clusterlayout.Cluster instance at 0x01BD8F80>}"""
    ...
    def newFun(self,val):
        ...
        myListValues = tempdict.values()
        myListKeys = tempdict.keys()
        
        data = vstack((array(myListValues)+ array([.5]),array(myListValues)))        
        centroids,_ = kmeans(data,1)
        centroidList =[]
        centroidList.append(centroids[:,0][0])
        centroidList.append(centroids[:,1][0])
        
        idx,_ = vq(data,centroids)
        
        #newList.append(data[idx==0,0][0])
        
        
        plot(data[idx==0,0],'ob',
             data[idx==1,0],'or')
        ...
        for k in range(len (myListValues)):
            
            if self.interruptRequested (): break
            
            dist1 = math.sqrt((centroidList[0]-myListValues[k])* (centroidList[0]-myListValues[k]))
            #print "******"
            
            #print centroidList[0]
            #print myListValues[k]
            #print dist1
            #print "#######"
            dist2 = math.sqrt((centroidList[1]-myListValues[k])* (centroidList[1]-myListValues[k]))            
            ...
            if dist1 < dist2:
                ...
                self.clusterMembership [int(myListKeys[myListValues.index(myListValues[k])])] = _cluster1
                _cluster1.addNode (int(myListKeys[myListValues.index(myListValues[k])]))
                self.unallocatedNodes.remove (int(myListKeys[myListValues.index(myListValues[k])]))
                #print "First"
            ...
            if dist2 < dist1:
                ...
                self.clusterMembership [int(myListKeys[myListValues.index(myListValues[k])])] = _cluster2
                _cluster2.addNode (int(myListKeys[myListValues.index(myListValues[k])]))
                print "1st"
                print myListKeys[myListValues.index(myListValues[k])]
                print centroidList[1]
                self.unallocatedNodes.remove (int(myListKeys[myListValues.index(myListValues[k])]))
        ...
        #self.root.cluster.view.mainThread.display
