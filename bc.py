    ...
    def eventFilter (self, anObject, event):
        if event.type () == QtCore.QEvent.KeyPress:
            if event.key () == QtCore.Qt.Key_Control:
                ...
                elif not isinstance (self.root.layoutType (), lag.LagPlot):
...
class TreeModel (QAbstractItemModel):
    ...
    def setData (self, index, value, role):
       ...
       # if item.data == "External Data":
       #     self.root.displayCancellableMessageBox ('External Data', "This Data has not been activated. Please contact your BC representative should you wish to activate it. It will still produce indicative displays, so please feel free to \'play\'.")           
...
class BrandCommunities (QMainWindow): 
    def __init__ (self, application, appname):
        ...
        # Lag Period # Analysis
        self.lagPlot = lag.LagPlot (self)

        if self.constants.facilitiesKey & self.constants.FACILITIES_TAB_LAG:
            self.defineLayoutType ('lag', self.lagPlot)
            self.mainPanel.addTab (self.lagPlot, 'Analysis')
    ...
    def setLayoutTypeById (self, index):
        ...
        if isinstance (self.layoutType (), Galaxy):
            ...
            self.linkQuantityTextField.setEnabled (False)
            self.arrowheadsCheckbox.setEnabled (False)
        elif isinstance (self.layoutType (), lag.LagPlot):
            print self.layoutType ()
            self.showAssociationsToolbox (False)
            self.linksCheckbox.setEnabled (False)
            self.linkQuantityTextField.setEnabled (False)
    ...
    def selectorPage (self, tabWidget):
        ...
        #_loadGlobalDatabase = QtGui.QCheckBox ('Load Global')
        #_b.addWidget (_loadGlobalDatabase)
        #QtCore.QObject.connect (_loadGlobalDatabase, QtCore.SIGNAL ('stateChanged (int)'), self.loadGlobalDatabse)
        
        #_layout.addStretch ()

        #bcComboBox = BCTimeseriesTransformComboBox ()
        distrolist = ['Client Data', 'Global Context Data','Client + Country Data']
        self.combo = QtGui.QComboBox()
        self.combo.addItems(distrolist)
        _layout.addWidget (self.combo, 1,0,1,2 )
        QtCore.QObject.connect (self.combo, QtCore.SIGNAL ('activated (int)'), self.loadGlobalDatabse)
        #self.combo.activated[str].connect(self.loadGlobalDatabse)

        _collapse = QPushButton ('Collapse all')
        _layout.addWidget (_collapse, 2, 0)
        QObject.connect (_collapse, SIGNAL ('clicked ()'), self.newSelector.collapse)

        _expand = QPushButton ('Expand all')
        _layout.addWidget (_expand, 2, 1)
    ...
    def applyConfiguration (self, config):
        ...
        try:
            self.galaxy.view.outerLinkCutoffSlider.slider.setValue (_config ['outerLinks'])
        except:
            self.configWarning ('Warning: outerLinks not found in configuration file.')
    ...
    def gatherConfigurationData (self):
        ...
        _config ['scenario'] = 0#self.scenarioSelectorCombo.currentIndex ()
        self.scenarioSelectorCombo.setCurrentIndex(0)
        ...
        _config ['outerLinks'] =  self.galaxy.view.outerLinkCutoffSlider.slider.value ()
...
""")
        ...
        self.galaxy.view.outerLinkCutoffSlider = Slider ('Outer Sieve:', 0, 255, 0, self.galaxy.view.adjustOuterLinkCutoff)
        self.galaxy.view.outerLinkCutoffSlider.setToolTip (r"""
Adjusts the cutoff value applied to links between the central node and all other nodes.
Links with coefficients less than the setting of this slider will not be displayed (even if the Show links checkbox has been checked).
Nor will they be considered in layout calculations.
If the link between the central node and any other node has been 'cut off' by the setting of this slider, that node will not be displayed.
...
The slider's range is between 0 and 100%, with 100% corresponding to the largest value of coefficient amongst all the central links.
""")
        _b.addWidget (self.galaxy.view.outerLinkCutoffSlider)
        ...
        _exportToCSVButton = QPushButton ('Export to CSV')
        _b.addWidget (_exportToCSVButton)
        _exportToCSVButton.setToolTip ('Export to CSV All visible nodes')
        QtCore.QObject.connect (_exportToCSVButton, QtCore.SIGNAL ('clicked ()'), self.exportToCsv)
        
        
        # 'Export visible nodes
        _forecastButton = QPushButton ('Forecast')
        _b.addWidget (_forecastButton)
        _forecastButton.setToolTip ('Export to CSV All visible nodes')
        QtCore.QObject.connect (_forecastButton, QtCore.SIGNAL ('clicked ()'), self.showForecast)
        
        # Add checkbox for forecast warning
        _showForecastWarning = QtGui.QCheckBox ('Show Confidence')
        _b.addWidget (_showForecastWarning)
        QtCore.QObject.connect (_showForecastWarning, QtCore.SIGNAL ('stateChanged (int)'), self.showWarning)
        ...
        # Amar add a new XY from Centrifuge button

        _xyFromCentrifugeButton = QPushButton ('Add From Centrifuge')
        _b.addWidget (_xyFromCentrifugeButton)
        _xyFromCentrifugeButton.setToolTip ('Show XY time series for the centrifuge central node')
        QtCore.QObject.connect (_xyFromCentrifugeButton, QtCore.SIGNAL ('clicked ()'), self.presetXY)
        ...
        _label = QLabel ('Cluster Number:')
        self.cluster.view.clusterSpinBox = QSpinBox ()
        self.cluster.view.clusterSpinBox.setValue (0)
        self.cluster.view.clusterSpinBox.setMinimum (0)
        self.cluster.view.clusterSpinBox.setMaximum (9999)
        QObject.connect (self.cluster.view.clusterSpinBox, SIGNAL ('valueChanged (int)'), self.clusterSpinBox)
        _b.addWidget (_label)
        _b.addWidget (self.cluster.view.clusterSpinBox)
        _b.addStretch ()
        
        #### Added by Dibyendu for lag period 
        # Lag plot
        _a = QWidget ()
        ...
        _b.addWidget (QLabel ('<b>Analysis</b>'))
        # 'Add' button
        _addButton = ToolButton (self.lagPlot, ':/images/Resources/add32x32.png', 'Add a new graph', self.lagPlot.addGraph)

        if self.constants.facilitiesKey & self.constants.FACILITIES_ANALYSIS_ADDANDDELETE:
            _b.addWidget (_addButton)
        ...
        self.futureSpinBox = QSpinBox ()
        self.futureSpinBox.setValue (0)
        self.futureSpinBox.setMinimum (0)
        self.futureSpinBox.setMaximum (9999)
        QObject.connect (self.futureSpinBox, SIGNAL ('valueChanged (int)'), self.futureAltered)
        ...
        return _widget
    def clusterSpinBox(self,val):
        self.clusterLayoutThread.newFun(val)
    def presetForecast (self):
        value = self.futureSpinBox.value()
        #print "future periods value: " + str(value)
        
        self.analysisPlot.presetForecast (self.centerNode ())
        
        self.futureAltered (value)

    def exportToCsv (self):
        self.analysisPlot.exportCSV ()  
        
    def showWarning(self,val):
        self.analysisPlot.showWarning(val)
           
    def showForecast(self):        
        self.analysisPlot.showRecentForecast()
    
    def presetXY (self):
        #self.xyPlot.view.zapAllAssociations ()
        self.xyPlot.view.presetXY(self.centerNode ())
    ...
    def futureAltered (self, value):
        ...
        self.lagPlot.extendTime (value, self.futureExtent)
    ...
    def loadGlobalDatabse(self,value):
        
        if value==1:
            self.scenarioSelectorCombo.changeScenario (0)
            text = str(self.scenarioSelectorCombo.currentText())
            print text
        if value==2:
            self.scenarioSelectorCombo.changeScenario (0)
            text = str(self.scenarioSelectorCombo.currentText())
            ...
            self.scenarioSelectorCombo.changeScenario (1)
            text = str(self.scenarioSelectorCombo.currentText())
            print text
    ...
    def leave (self):
        ...
        os.kill (os.getpid (), 9)
class BCTimeseriesTransformComboBox (QtGui.QComboBox):
    def __init__ (self):
